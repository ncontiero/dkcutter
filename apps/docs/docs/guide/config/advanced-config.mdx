---
description: Configure advanced DKCutter prompts using object properties for custom messages, regex validation, conditional disabling, and multiselect choices
---
...
## DKCutter Specific Configuration (`_dkcutter`) <Badge text="DKCutter v6.2.0+" type="info" />
...
In this example, the template requires DKCutter version 7.0.0 or higher. If you're running it with a Node-based package manager (like `npm`, `yarn`, or `pnpm`), it will enforce Node >= 24.0.0. If you're running it with `bun`, it will enforce Bun >= 1.3.0.

### Copying Files Without Rendering

By default, DKCutter will try to render all files in the template directory. If you have files that should not be rendered (e.g., specific text files containing raw Jinja/Nunjucks syntax), you can use the `copyWithoutRender` property to copy them directly without processing.

> [!NOTE]
> You don't need to add binary files (like `.png`, `.jpg`, `.ico`, etc.) to this list! Under the hood, DKCutter automatically uses the [isbinaryfile](https://github.com/gjtorikian/isBinaryFile) library to identify binary files and just copies them without rendering.

This property accepts an array of strings containing glob patterns. Under the hood, DKCutter uses [tinyglobby](https://github.com/SuperchupuDev/tinyglobby) to match the files.

```json title="dkcutter.json"
{
  "_dkcutter": {
    "copyWithoutRender": ["templates/**/*.html", "scripts/**/*.sh"]
  },
  "projectName": "My Awesome Project"
}
```
...
If you have files or directories in your template that you do not want to be copied or rendered in the generated project, you can use the `ignore` property.

Like `copyWithoutRender`, this property accepts an array of strings containing glob patterns and is evaluated using [tinyglobby](https://github.com/SuperchupuDev/tinyglobby).

```json title="dkcutter.json"
{
  "_dkcutter": {
    "ignore": [".github/workflows/internal.yml", "docs/**", "temp_*"]
  },
  "projectName": "My Awesome Project"
}
```
